...
const Navbar = ({ activeRoute }) => {
    ...
    function renderRoutes() {
        siteRoutes.map((i) => {
            console.log(i.replace('/', "").charAt(0).toUpperCase() + i.substring(1))
            if (i == activeRoute) {
                return (
                ...
                return <Link to={i}><li className='p-3 hover:bg-gray-600 hover:text-white rounded-md transition-all cursor-pointer'>{i.replace('/', "").charAt(0).toUpperCase() + i.substring(1)}!</li></Link>
            }
        })
    }
    ...
    return (
        <div className="bg-primary-heavy p-5 h-30 flex items-center justify-between drop-shadow-md">
            ...
            <div className={`absolute xl:hidden top-24 left-0 w-full bg-primary-heavy flex flex-col items-center gap-6 font-semibold text-lg transform transition-transform ${isMenuOpen ? "opacity-100" : "hidden"}`}
                ...
                <Link to={"/home"}><li className='list-none w-full text-center p-4 hover:bg-gray-600 hover:text-white transition-all cursor-pointer font-bold'>Home</li></Link>
                <Link to={"/home"}><li className='list-none w-full text-center p-4 hover:bg-gray-600 hover:text-white transition-all cursor-pointer'>Explore</li></Link>
